...
async def run_first_part(playwright: Playwright, start_date: str, end_date: str, day_start: str, day_end: str, month_end_number: str, download_path: str, log_df: pd.DataFrame) -> pd.DataFrame:
    ...
    context = await browser.new_context(storage_state=r"C:\Users\bechara.9\Copias Scripts\ETL Automation\datapowerMI.json")
    ...
    await page.locator("#dialog-0").get_by_placeholder("Start date").click()
...
def send_email(log_df):
    try:
        ...
        if not failure_logs.empty:
            ...
            mail.Body = "Todos os downloads de BPO People MI foram bem-sucedidos.\n\nObrigado."
