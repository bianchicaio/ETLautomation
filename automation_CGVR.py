...
def calculate_weeks():
    ...
    return {
        ...
        "week_before_last": (week_before_last_start, week_before_last_end)
    }
...
async def download_data(page, download_path, start_date, end_date):
    ...
    await page.frame_locator("#data-power-app iframe").locator("div").filter(has_text=re.compile(r"^CGVR$")).first.click()
    print("Selecionou CGVR")
...
async def main():
    ...
    start_date_last_week, end_date_last_week = weeks["last_week"]
    await process_download(start_date_last_week, end_date_last_week, download_path)

    # Realizar o download da semana retrasada
    start_date_week_before_last, end_date_week_before_last = weeks["week_before_last"]
